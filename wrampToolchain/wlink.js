...
async function WLinkModule(moduleArg = {}) {
  var moduleRtn;
...
function findWasmBinary() {
  ...
  if (Module['locateFile']) {
    return locateFile('wlink.wasm');
  }

  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('wlink.wasm', import.meta.url).href;
...
// Begin JS library code
  ...
  put_char(tty, val) {
          if (val === null || val === 10) {
            triggerPopover(UTF8ArrayToString(tty.output));
            tty.output = [];
          } else {
...
var _wlink = Module['_wlink'] = makeInvalidEarlyAccess('_wlink');
...
function assignWasmExports(wasmExports) {
  assert(typeof wasmExports['wlink'] != 'undefined', 'missing Wasm export: wlink');
  ...
  _wlink = Module['_wlink'] = createExportWrapper('wlink', 1);
